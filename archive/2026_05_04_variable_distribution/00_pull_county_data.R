...
acs_data <- acs_data %>%
  ...
  dplyr::select(geoid=GEOID,poverty_count,poverty_denominator,poverty_pct)
...
total_population %<>% dplyr::select(-c(variable, moe))
...
variables %<>% dplyr::select(-estimate_label, -total_label, -concept)
...
# see_something <- county %>% 
#   summarise(
#     mean_pct_pov = round(mean(poverty_pct ,na.rm = T),2),
#     median_pct_pov = round(median(poverty_pct ,na.rm = T),2),
#     sd_pct_pov = round(sd(poverty_pct ,na.rm = T),2),
#     min_pct_pov = round(min(poverty_pct ,na.rm = T),2),
#     max_pct_pov = round(max(poverty_pct ,na.rm = T),2),
#     iqr_pct_pov = round(IQR(poverty_pct ,na.rm = T),2))

## add in the county-level mortality (from NCHS) ##-----------------------------------
